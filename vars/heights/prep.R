...
## Last-Updated: Tue Mar 24 14:02:55 2015 (-0400)
##           By: Noah Peart
######################################################################
...
prep_hh <- function(dat, yr, spec, can_func="can_hh") {
    ...
    has_dbh <- interp(~!is.na(dbh), dbh=as.name(dbh))
    has_ht <- interp(~!is.na(ht), ht=as.name(ht))
    not_south <- interp(~!grepl("^S", TRAN))
    not_E335_16 <- interp(~!(TRAN == "E335" & TPLOT == 16))

    res <- dat %>% filter_(~ELEVCL == "HH", is_alive, has_dbh, has_ht, not_south,
                           not_E335_16, ~SPEC %in% toupper(spec))
